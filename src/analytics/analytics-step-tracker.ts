...
export interface StepTrackerConfig {
    shouldBackfillPreviousSteps?: boolean;
    stepsOrder?: AnalyticStepType[];
}
...
export default class AnalyticsStepTracker implements StepTracker {
    ...
    private viewedSteps: { [key in AnalyticStepId]?: boolean; } = {};
    private analyticStepOrder: AnalyticStepType[] = [
        'customer',
        'shipping',
        'billing',
        'payment',
    ];
    private shouldBackfillPreviousSteps: boolean = false;

    constructor(
        private checkoutService: CheckoutService,
        private storage: StorageFallback,
        private analytics: AnalyticsTracker,
        { shouldBackfillPreviousSteps, stepsOrder }: StepTrackerConfig = {}
    ) {
        if (shouldBackfillPreviousSteps !== undefined) {
            this.shouldBackfillPreviousSteps = shouldBackfillPreviousSteps;
        }

        if (stepsOrder !== undefined && stepsOrder.length) {
            this.analyticStepOrder = stepsOrder;
        }
    }
    ...
    private backfill(stepId: AnalyticStepId): void {
        if (!this.shouldBackfillPreviousSteps) {
            return;
        }

        for (const i of this.analyticStepOrder) {
